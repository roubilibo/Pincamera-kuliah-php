...
class fungsi_php
{
				                      	...
				                      	if (($ambil[1])!="") 
						{
					             ...
					             else
					             {
				                    ...
				                    <img src='".$data1['location']."' data-toggle='modal' data-target='.bs-example-modal-sm".$counter."' class='img-responsive'>
				                      ...
				                      <p style='text-align: right; padding-right:5%;' data-toggle='modal' data-target='.bs-example-modal-sm".$counter."'>
				                          <i class='glyphicon glyphicon-eye-open'></i> ".$data1['lihat']." view
				                      </p>
				                  </div>
				                </div>
	...
	function cari()
	{
		...
		$lokasi1=mysqli_query($koneksi,"SELECT profil.location,upload.judul_post, upload.keterangan,upload.location,upload.lihat, upload.id_post, upload.tag FROM profil,upload WHERE upload.id_user=profil.id_user AND upload.tag LIKE '%".$_POST['kunci']."%'");
	...
	function koleksi()
	{
		...
		$query=mysqli_query($koneksi,"SELECT profil.location,upload.judul_post, upload.keterangan,upload.location,upload.lihat, upload.id_post, upload.tag FROM profil,upload WHERE upload.id_user='$id_user' and upload.id_user=profil.id_user");
		$cek=mysqli_num_rows($query);
				                      	...
				                      	if (($ambil[1])!="") 
						{
					             ...
					             else
					             {
				                    ...
				                    <img src='".$data['location']."' data-toggle='modal' data-target='.bs-example-modal-sm".$counter."' class='img-responsive'>
				                      ...
				                      <p style='text-align: right; padding-right:5%;' data-toggle='modal' data-target='.bs-example-modal-sm".$counter."'>
				                          <i class='glyphicon glyphicon-eye-open'></i> ".$data['lihat']." view
				                      </p>
				                  </div>
				                </div>
				                <div class='modal fade bs-example-modal-sm".$counter."' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'>
